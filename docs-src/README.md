...
```
docs/
└── index.html            ← the published page (served by GitHub Pages)

docs-src/                 ← this folder (not served)
├── FLATTEN.md            ← procedure for regenerating docs/index.html
├── README.md             ← this file
└── source-standalone.html ← the recipe used to regenerate docs/index.html
```
...
| File | Role |
|---|---|
| `source-standalone.html` | **The recipe.** A single-file HTML with the doc's content stored as JavaScript data structures, plus a Mermaid runtime that renders the six diagrams to SVG at page load. Not served by Pages — kept here so `docs/index.html` can be regenerated when structural changes are needed. |
| `FLATTEN.md` | **The regeneration procedure.** Explains how to turn `source-standalone.html` into `docs/index.html`. Refer to it before any re-flatten. |

## Editing the doc

**For most changes — prose, tunable values, `file:line` citations, table
cells, CSS tweaks — edit `docs/index.html` directly.** It's plain HTML with
real markup; grep and edit work exactly as you'd expect.

**For diagram label changes** (e.g., renaming a CLI verb that appears
inside a diagram box) — each `<svg>` in `docs/index.html` is preceded by an
...
edit, download the SVG, and swap it into `docs/index.html` in place of the
...
The published Pages URL updates automatically once `docs/index.html` is
